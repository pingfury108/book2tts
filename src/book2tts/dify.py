...
def llm_parse_text(text: str, api_key: str, files=None, base_api=BASE_API) -> str:
    ...
    completion_response = completion_client.create_completion_message(
        inputs={"query": text}, response_mode="blocking", user="book2tts", files=files
    )
...
def llm_parse_text_streaming(text: str, api_key: str, files=None, base_api=BASE_API):
    completion_client = CompletionClient(api_key)
    completion_client.base_url = base_api

    # Create Completion Message using CompletionClient
    completion_response = completion_client.create_completion_message(
        inputs={"query": text}, response_mode="streaming", user="book2tts", files=files
    )
...
def file_upload(api_key: str, files, base_api=BASE_API):
    ...
    completion_response = completion_client.file_upload(user="book2tts", files=files)
    completion_response.raise_for_status()
